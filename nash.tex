...
\textbf{1.} Program started. Nash starts executing bytecode of Scheme program
with interpreter. The interpretation continues until hot loop is detected.
\textbf{2.} A hot loop was detected by the interpreter. The interpreter
continue the evaluation of instructions with recording the bytecode and the
values from current stack.  \textbf{3.} The bytecode IP reached to the
beginning of the loop. The recording ends and traced information are passed to
Nash compiler.  \textbf{4.} Compilation finished with native code
generation. Control flow gets back to Nash Interpreter. Bytecode
interpretation continues from the IP where the recording has
ended. \textbf{5.}  Interpreter encountered the bytecode IP of beginning of
the compiled loop. This time compiled native code exists for the IP.\@ The
interpreter executes the native code.  \textbf{6.} A guard in the native code
has failed. Native code go through a bailout code to recover the state of the
interpreter, control flow gets back to interpreter.  \textbf{7.}  Interpreter
reached to the end of bytecode and the program terminates. Before reaching to
the end of bytecode, transition \textbf{2} and \textbf{5} are repeatedly taken
as necessary.
